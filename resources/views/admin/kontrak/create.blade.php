...
<div class="max-w-5xl mx-auto space-y-6">
    <div>
        <h1 class="text-3xl font-black text-white">Tambah Kontrak Perkuliahan</h1>
        <p class="text-slate-400 mt-1">Lengkapi informasi kontrak perkuliahan</p>
    </div>
    ...
    <form action="{{ route('kontrak.store') }}" method="POST"
        ...
        <div>
            ...
            <input type="text" name="judul_kontrak" value="{{ old('judul_kontrak') }}" required
        ...
        <div class="grid md:grid-cols-3 gap-5">
            <div>
                ...
                <select name="rps_id"
                    ...
                    @foreach($rps as $item)
                        <option value="{{ $item->id }}" {{ old('rps_id') == $item->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <textarea name="deskripsi_mata_kuliah" rows="3"
                      class="w-full rounded-2xl bg-slate-950/70 border border-white/10 px-4 py-3 text-white outline-none focus:ring-2 focus:ring-cyan-400">{{ old('deskripsi_mata_kuliah') }}</textarea>
        </div>

        <div>
            <label class="block mb-2 font-bold text-slate-300">Jadwal Kuliah</label>
            <input type="text" name="jadwal" value="{{ old('jadwal') }}" placeholder="Contoh: Senin, 08.00 - 10.00"
                   class="w-full rounded-2xl bg-slate-950/70 border border-white/10 px-4 py-3 text-white outline-none focus:ring-2 focus:ring-cyan-400">
        </div>
        ...
        @foreach([
            ...
            <div>
                ...
                <textarea name="{{ $name }}" rows="3"
                          class="w-full rounded-2xl bg-slate-950/70 border border-white/10 px-4 py-3 text-white outline-none focus:ring-2 focus:ring-cyan-400">{{ old($name) }}</textarea>
            </div>
        @endforeach
